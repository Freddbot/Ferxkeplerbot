...
const handler = async (m, { conn, args }) => {
   ...
   let txt = `           *乂  B O T  -  S C R I P T  乂*\n\n`;
      ...
      txt += `◦  *Url:* ${json?.html_url || 'https://github.com/Buitreofc/Buitre-BotLite-MD'}\n\n`;
      txt += `	 ${json?.forks_count || '-'} Forks · ${json?.stargazers_count || '-'} Stars · ${json?.open_issues_count || '-'} Issues`;
   await conn.sendMessage(m.chat, {text: txt.trim(), mentions: [...txt.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + '@s.whatsapp.net'), contextInfo: {forwardingScore: 9999999, isForwarded: true, mentionedJid: [...txt.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + '@s.whatsapp.net'), "externalAdReply": {"showAdAttribution": true, "containsAutoReply": true, "renderLargerThumbnail": true, "title": global.titulowm2, "containsAutoReply": true, "mediaType": 1, "thumbnail": imagen6, "mediaUrl": `https://github.com/Buitreofc/Buitre-BotLite-MD`, "sourceUrl": `https://boxmineworld.com`}}}, {quoted: m});
};
handler.help = ['scbot'];
handler.tags = ['info'];
